...
using RegValue = long long int;
using Address = unsigned long long int;

enum RegControl { rc_before_jump, rc_after_jump, rc_no_control };

// ==== Necessary evils end ====

// 'lite' version of emitter interface found in dyninstAPI
class Emitter {

public:
  virtual ~Emitter() {}
  virtual unsigned emitIf(Register expr_reg, Register target, RegControl rc,
                          codeGen &gen) = 0;
  virtual void emitOp(unsigned opcode, Register dest, Register src1,
                      Register src2, codeGen &gen) = 0;
  virtual void emitOpImm(unsigned opcode1, unsigned opcode2, Register dest,
                         Register src1, RegValue src2imm, codeGen &gen) = 0;
  virtual void emitRelOp(unsigned op, Register dest, Register src1,
                         Register src2, codeGen &gen, bool s) = 0;
  virtual void emitRelOpImm(unsigned op, Register dest, Register src1,
                            RegValue src2imm, codeGen &gen, bool s) = 0;
  virtual void emitDiv(Register dest, Register src1, Register src2,
                       codeGen &gen, bool s) = 0;
  ...
  virtual void emitDivImm(Register dest, Register src1, RegValue src2imm,
                          codeGen &gen, bool s) = 0;
  virtual void emitLoad(Register dest, Address addr, int size,
                        codeGen &gen) = 0;
  virtual void emitLoadConst(Register dest, Address imm, codeGen &gen) = 0;
  virtual void emitLoadIndir(Register dest, Register addr_reg, int size,
                             codeGen &gen) = 0;
  virtual bool emitCallRelative(Register, Address, Register, codeGen &) = 0;
  virtual bool emitLoadRelative(Register dest, Address offset, Register base,
                                int size, codeGen &gen) = 0;
  // virtual void emitLoadShared(opCode op, Register dest, const image_variable
  // *var, bool is_local, int size, codeGen &gen, Address offset) = 0;

  virtual void emitLoadFrameAddr(Register dest, Address offset,
                                 codeGen &gen) = 0;
  ...
  virtual void emitLoadOrigRegRelative(Register dest, Address offset,
                                       ...
                                       bool store) = 0;
  virtual void emitLoadOrigRegister(Address register_num, Register dest,
                                    codeGen &gen) = 0;

  virtual void emitStoreOrigRegister(Address register_num, Register dest,
                                     codeGen &gen) = 0;

  virtual void emitStore(Address addr, Register src, int size,
                         codeGen &gen) = 0;
  virtual void emitStoreIndir(Register addr_reg, Register src, int size,
                              codeGen &gen) = 0;
  virtual void emitStoreFrameRelative(Address offset, Register src,
                                      Register scratch, int size,
                                      codeGen &gen) = 0;
  virtual void emitStoreRelative(Register source, Address offset, Register base,
                                 int size, codeGen &gen) = 0;
  // virtual void emitStoreShared(Register source, const image_variable *var,
  // bool is_local, int size, codeGen &gen) = 0;
  ...
  // virtual Register emitCall(opCode op, codeGen &gen, const
  ...
  // instPoint::Type pt_type, opCode op, bool addr_of, codeGen &gen) = 0;
  virtual void emitASload(int ra, int rb, int sc, long imm, Register dest,
                          int stackShift, codeGen &gen) = 0;
  ...
  virtual void emitStoreImm(Address addr, int imm, codeGen &gen,
                            bool noCost) = 0;
  ...
  // virtual bool emitPLTCall(func_instance *, codeGen &) { assert(0); return
  // false;} virtual bool emitPLTJump(func_instance *, codeGen &) { assert(0);
  // return false;}
  //
  // virtual bool emitTOCJump(block_instance *, codeGen &) { assert(0); return
  // false; } virtual bool emitTOCCall(block_instance *, codeGen &) { assert(0);
  // return false; }
